...
import { getProposalScores, getProposal, sha256 } from '../utils';
...
const getSubscribedWallets = async space => {
  const subscriptions = await db.queryAsync('SELECT * FROM notificationSubscriptions WHERE space = ?', [space]);
...
const sendPushNotification = async event => {
  ...
  const proposal: { [key: string]: any } | null = await getProposal(event.id.replace('proposal/', ''));
  if (!proposal) {
    console.log('[notification] Proposal not found', event.id);
    return;
  }
...
async function sendEvent(event, to) {
  // TODO: if discord bot, call handleDiscordMessages function instead of sending a API request
